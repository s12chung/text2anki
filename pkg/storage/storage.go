...
func (s Signer) SignPut(table, field string, exts []string) ([]PresignedHTTPRequest, string, error) {
	...
	for i, ext := range exts {
		req, err := s.api.SignPut(path.Join(table, field, stringID, strconv.Itoa(i)+ext))
		if err != nil {
			return nil, "", err
		}
		reqs[i] = req
